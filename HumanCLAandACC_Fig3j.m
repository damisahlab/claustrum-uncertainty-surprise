...
load(fullfile(data_path, 'Fig3j_ACC_OutcomeNeurons.mat'))
addpath(fullfile(fileparts(pwd), 'OnPathCodes')) % add path to helper tools 

% load data
hit = Fig3j_ACC_OutcomeNeurons{1,1};
miss = Fig3j_ACC_OutcomeNeurons{1,2};
inesp = Fig3j_ACC_OutcomeNeurons{1,3};
unityLine = Fig3j_ACC_OutcomeNeurons{1,4};
...
legend('crash-specific(4)','avoid-specific(6)','non-specific(3)')
xlim([-0.5 8])
ylim([-0.5 8])
hold off

sgt= sgtitle('Figure 3j. Outcome-Specific ACC Neurons');
sgt.FontSize = 13;
