...
load(fullfile(data_path, 'Fig3i_ACC_OutcomeNeurons.mat'))
addpath(fullfile(fileparts(pwd), 'OnPathCodes')) % add path to helper tools 

% load data
rateData = Fig3i_ACC_OutcomeNeurons{1,1};
...
%stats data
statsData = Fig3i_ACC_OutcomeNeurons {1,2};
preApp = statsData(:,1);
app = statsData(:,2);
avoid = statsData(:,3);
crash = statsData(:,4);
...
sgt= sgtitle('Figure 3i. Outcome-specific ACC Neurons');
sgt.FontSize = 13;
