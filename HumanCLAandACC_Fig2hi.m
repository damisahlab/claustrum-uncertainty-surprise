...
load(fullfile(data_path, 'Fig2hi_CLA_AppearOutcomeNeurons.mat'))
addpath(fullfile(fileparts(pwd), 'OnPathCodes')) % add path to helper tools 

% load data
rateData = Fig2hi_CLA_AppearOutcomeNeurons{1,1};
...
%stats data
statsData = Fig2hi_CLA_AppearOutcomeNeurons {1,2};
preApp = statsData(:,1);
app = statsData(:,2);
avoid = statsData(:,3);
crash = statsData(:,4);
...
sgt= sgtitle('Figure 2h and 2i. Appear & Outcome Claustrum Neurons');
sgt.FontSize = 13;
