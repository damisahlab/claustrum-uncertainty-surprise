%-----------------------------------------------------------------------------------------
% Script for appear-specific claustrum neurons
%
...
load(fullfile(data_path, 'Fig3fg_CLA_AppearSpecificNeurons.mat'))
addpath(fullfile(fileparts(pwd), 'OnPathCodes')) % add path to helper tools 

% load data
rateData = Fig3fg_CLA_AppearSpecificNeurons{1,1};
...
%stats data
statsData = Fig3fg_CLA_AppearSpecificNeurons{1,2};
preApp = statsData(:,1);
app = statsData(:,2);
avoid = statsData(:,3);
crash = statsData(:,4);
...
sgt= sgtitle('Figure 3f and 3g. Appear-Specific Claustrum Neurons');
sgt.FontSize = 13;
