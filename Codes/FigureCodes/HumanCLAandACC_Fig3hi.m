%-----------------------------------------------------------------------------------------
% Script for analysis of appear & outcome claustrum neurons 
%
...
load(fullfile(data_path, 'Fig3hi_CLA_AppearOutcomeNeurons.mat'))
addpath(fullfile(fileparts(pwd), 'OnPathCodes')) % add path to helper tools 

% load data
rateData = Fig3hi_CLA_AppearOutcomeNeurons{1,1};
...
%stats data
statsData = Fig3hi_CLA_AppearOutcomeNeurons{1,2};
preApp = statsData(:,1);
app = statsData(:,2);
avoid = statsData(:,3);
crash = statsData(:,4);
...
sgt= sgtitle('Figure 3h and 3i. Appear & Outcome Claustrum Neurons');
sgt.FontSize = 13;
