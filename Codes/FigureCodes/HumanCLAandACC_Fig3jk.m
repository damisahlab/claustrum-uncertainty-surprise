...
load(fullfile(data_path, 'Fig3jk_CLA_OutcomeNeurons.mat'))
addpath(fullfile(fileparts(pwd), 'OnPathCodes')) % add path to helper tools 

% load data
rateData = Fig3jk_CLA_OutcomeNeurons{1,1};
...
%stats data
statsData = Fig3jk_CLA_OutcomeNeurons {1,2};
preApp = statsData(:,1);
app = statsData(:,2);
avoid = statsData(:,3);
crash = statsData(:,4);
...
sgt= sgtitle('Figure 3j and 3k. Outcome-Specific Claustrum Neurons');
sgt.FontSize = 13;
