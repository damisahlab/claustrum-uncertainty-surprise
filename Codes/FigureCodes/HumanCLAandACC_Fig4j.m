...
load(fullfile(data_path, 'Fig4j_ACC_OutcomeNeurons.mat'))
addpath(fullfile(fileparts(pwd), 'OnPathCodes')) % add path to helper tools 

% load data
hit = Fig4j_ACC_OutcomeNeurons{1,1};
miss = Fig4j_ACC_OutcomeNeurons{1,2};
inesp = Fig4j_ACC_OutcomeNeurons{1,3};
unityLine = Fig4j_ACC_OutcomeNeurons{1,4};
...
sgt= sgtitle('Figure 4j. Outcome-Specific ACC Neurons');
sgt.FontSize = 13;
