...
def plot_individual_neurons(results, method_name, time_point, save_dir,
    ...
    for entry in results:
        ...
        fig_path = os.path.join(save_dir, rf"{brain_region}_{method_name}_{time_point}_{file_name}_{behavior_name}.svg")
...
def plot_pval_heatmap(pval, neuron_labels, columns_to_keep, brain_region, 
    ...
    ax_heat.tick_params(axis='x', bottom=False, labelbottom=True)
    ax_heat.set_yticks(np.arange(n_behaviors)+0.5)
    ax_heat.set_xticks(np.arange(n_neurons)+0.5) 
    ax_heat.set_yticklabels(columns_to_keep[::-1], fontsize=5, fontname='Arial')
    ax_heat.set_xticklabels(neuron_labels_sig, fontsize=5, fontname='Arial', rotation=90)
...
def process_and_cluster(curves, title, brain_region, n_points=200):
    ...
    plt.tight_layout()
    plt.savefig(rf"{save_path}\{brain_region}_clusters_{title}.svg",
                format='svg')
    plt.close()   
...
sub_neurons = pd.read_csv(fr'define good neuron list path\good_neurons_{brain_region}.csv')
...
# Build subfolders
asteroid_path = os.path.join(save_path, "Rasters", "asteroid")
intertrial_path = os.path.join(save_path, "Rasters", "intertrial")
# Create directory if it doesn't exist
os.makedirs(asteroid_path, exist_ok=True)
os.makedirs(intertrial_path, exist_ok=True)
...
for sub in subjects:    
    ...
    mat_dir = os.path.join(f'{data_path}/{brain_region}/', f'{sub}')
    for i, file_name in enumerate(file_names):        
        ...
        st = st['spike_times']
        # load behavior
        file_path = os.path.join(f'{data_path}/{brain_region}/', f'{sub}/{brain_region}_{sub}.xlsx')
        ...
        # load y-position
        file_path = os.path.join(rf'{data_path}/yPos', f'{sub}_gameinfo_yPos.csv')
        ...
        for idx in range(3):
            ...
            if p_it_mi[idx] < 0.05:
                ...
                plot_raster(trials=st, fr=fr, behavior=np.array(x),
                    ...
                    folder_path=rf'{save_path}/Rasters/intertrial/')
        ...
        for idx in range(8):
            ...
            if p_a_mi[idx] < 0.05:
                ...
                plot_raster(trials=st, fr=fr, behavior=np.array(x),
                    time_axis=np.linspace(-4, 2, fr.shape[1]),
                    column=behavior_name, brain_region=brain_region, file_name=file_name,
                    folder_path=rf'{save_path}/Rasters/asteroid/')

mapping_df = pd.DataFrame(file_cnt_mapping)

#%% Save data to generate Chord Diagrams
save_results_to_excel(brain_region=brain_region,
    result_mi_it=result_mi_it, result_mi_a=result_mi_a, save_folder=save_path) 
...
plot_pval_heatmap(
    ...
    brain_region, "MI_score", save_path, 'Intertrial', scale_intertrial)

# Asteroid period
plot_pval_heatmap(
    np.vstack(p_values_a_mi), neuron_labels,
    columns_to_keep, brain_region, "MI_score", save_path, 'Asteroid', scale_asteroid)
...
#%% Separate neurons by shape (z-scored)
