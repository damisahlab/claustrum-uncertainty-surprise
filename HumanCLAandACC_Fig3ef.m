...
load(fullfile(data_path, 'Fig3ef_ACC_AppearSpecificNeurons.mat'))
addpath(fullfile(fileparts(pwd), 'OnPathCodes')) % add path to helper tools 

% load data
rateData = Fig3ef_ACC_AppearSpecificNeurons{1,1};
...
%stats data
statsData = Fig3ef_ACC_AppearSpecificNeurons{1,2};
preApp = statsData(:,1);
app = statsData(:,2);
avoid = statsData(:,3);
crash = statsData(:,4);
...
sgt= sgtitle('Figure 3e and 3f. Appear-Specific ACC Neurons');
sgt.FontSize = 13;
