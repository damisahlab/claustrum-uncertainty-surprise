...
def scatter_plot(x_ut, y_ut, path, title, label=None, dpi=20, scatter_color='#0077b6', title_add=False): 
    ...
    y_min, y_max = x_min, x_max #y_ut.min(), y_ut.max()
...
#     fig, ax = plt.subplots(figsize=(12, 12))
#     scatter = ax.scatter(df_ut["GT_Tensor"], df_ut["PRED_Tensor"], s=50, c=scatter_color, alpha=0.6)

#     # Plot the diagonal line y = x
#     line_ideal, = ax.plot([-0.01, 1.01], [-0.01, 1.01], color='black', linestyle='--', label='Ideal Fit (y = x)')

#     # Fit linear regression and plot the regression line (optional)
#     lm = LinearRegression()  
#     lm.fit(x_ut.reshape(-1, 1), y_ut.reshape(-1, 1)) 
#     x_range = np.linspace(-0.05, 1.05, 100)
#     y_range = lm.predict(x_range.reshape(-1, 1)) 

#     # Compute Spearman correlation and R² score correctly
#     spearman_corr, _ = spearmanr(x_ut, y_ut)   
#     r2 = r2_score(x_ut, y_ut) 
...
#     # Create a custom legend entry for Spearman correlation and R²
#     if title_add:
#         empty_handle = Line2D([], [], linestyle='none', label=f'    Spearman Correlation: {spearman_corr}')
#         # empty_handle = Line2D([], [], linestyle='none', label=f'Spearman Corr.: {spearman_corr}  \nR² Score: {r2}')
...
#         handles = [empty_handle]
#         labels = [h.get_label() for h in handles] 
#         ax.legend(handles=handles, labels=labels, loc='upper left', handlelength=0, handletextpad=0,
#             fontsize=40, frameon=False, bbox_to_anchor=(0, 1.12))  # Slightly higher


#     plt.tight_layout(pad=0) 
#     # pdf_path = path+'pdf/'
#     png_path = path #+'scatter/'
#     # mkdir_fun(pdf_path)
#     # mkdir_fun(png_path)
#     plt.savefig(f"{png_path}_{label}.png", dpi=dpi)  
#     # plt.savefig(f"{pdf_path}{label}.pdf", dpi=50)  
#     plt.close()
